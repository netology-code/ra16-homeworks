...
Вот переработанный Markdown с техническим заданием, шаблоном решения и чек-листом для задания «Календарь» в нужном формате, с учетом твоих пожеланий:

---

# Календарь

Создать React-компонент `Calendar` (функциональный компонент), который отображает текущую дату и месяц, как показано на изображении:
![Внешний вид компонента](./assets/preview.png)

## Пример использования

```jsx
const now = new Date(2017, 2, 8); // 8 марта 2017
